...
class Model(nn.Module):
    ...
    def __init__(self, args, configs_list, pretrain=False):
        ...
        for i in range(self.num_task):
            ...
            if dataset_name not in self.prompt_tokens:
                ...
                self.mask_tokens[dataset_name] = torch.zeros(
                    1, enc_in, 1, args.d_model)

            if configs_list[i][1]['task_name'] == 'classification':
                self.category_tokens[task_data_name] = torch.zeros(
                    1, enc_in, configs_list[i][1]['num_class'] * configs_list[i][1]['num_labels'], args.d_model)
                torch.nn.init.normal_(
        ...
        for i in range(self.num_task):
            task_data_name = configs_list[i][0]
            if configs_list[i][1]['task_name'] == 'classification':
                self.cls_nums[task_data_name] = [
                    configs_list[i][1]['num_labels'], configs_list[i][1]['num_class']
                ]
    ...
    def classification(self, x, x_mark, task_id):
        ...
        #! Reshape the output to have [num_labels, num_class] shape
        x = x.reshape(-1, self.cls_nums[task_data_name]
                      [0], self.cls_nums[task_data_name][1])
