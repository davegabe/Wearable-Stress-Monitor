...
from anomaly_detection import test_tadgan
...
def train_tadgan(
    ...
    TRAINING LOOP
    """
    for epoch in tqdm(range(n_epochs), position=0, leave=True, desc="Epochs"):
        n_critics = 5
        ...
        wandb.log(
            {
                "cx_loss": cx_epoch_loss[-1],
                "cz_loss": cz_epoch_loss[-1],
                "decoder_loss": decoder_epoch_loss[-1],
                "hyp_loss": hyp_dec_loss[-1] if params.hyperbolic else 0,
                "eucl_loss": eucl_dec_loss[-1],
                "epoch": actual_epoch,
            }
        )
        ...
        if (actual_epoch % 5 == 0) or (actual_epoch == (n_epochs - 1)):
...
def train(train_loader, test_loader, params, config_path, read_path):
    """
