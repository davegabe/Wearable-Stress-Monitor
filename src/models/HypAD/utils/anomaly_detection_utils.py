...
from utils.utils import adjustment
...
def multivariate_anomaly_detection(
    recons_signal, true_signal, params, combination, critic_score, path, known_anomalies, labels
):
    if not params.hyperbolic:
        """
        euclidean reconstruction error
    ...
    # If no anomalies are detected, we create an empty dataframe
    if len(intervals) == 0:
        pred_anomalies = pd.DataFrame(columns=["start", "end", "score"])
    # Otherwise, we create a dataframe with the detected anomalies
    else:
        pred_anomalies = pd.DataFrame(intervals, columns=["start", "end", "score"])

    # Compute AUC metrics
    auc_roc = roc_auc_score(labels, final_scores)
    auc_pr = average_precision_score(labels, final_scores)

    # Get labels final list
    ...
    gt, pred = adjustment(labels, pred)
...
def compute_metrics(known_anomalies, pred_anomalies, start, end):
    """
    computing F1 score and gmean
    """
    _, fp, fn, tp = contextual_confusion_matrix(
        known_anomalies, pred_anomalies, 
        start, end,
        weighted=False
    )
    # Log wandb metrics
    wandb.log({"fp": fp, "fn": fn, "tp": tp})

    if tp + fp == 0:
        precision = np.nan # Not defined
    else:
        precision = tp / (tp + fp)

    if tp + fn == 0:
        recall = np.nan # Not defined
    else:
        recall = tp / (tp + fn)

    if precision is None or recall is None or precision + recall == 0:
        F1 = np.nan # Not defined
    else:
...
def _overlap(expected, observed):
    ...
    Args:
        expected (tuple): Expected anomaly (start, end).
        observed (tuple): Detected anomaly (start, end).
    """
...
def _ws(expected, observed, start, end):
    """
    Evaluate the confusion matrix between the ground truth and the detected anomalies for each data point.

    Args:
        expected (list of tuples): Ground truth.
        observed (list of tuples): Detected anomalies.
        start (int): Minimum timestamp of the original data.
        end (int): Maximum timestamp of the original data.

    Returns:
        tuple: Number of true negative, false positive, false negative, true positive.    
    """
    # Initialize the confusion matrix
    tn, tp, fp, fn = 0, 0, 0, 0

    # Get list of indices for the expected anomalies
    expected_indices = []
    for i in range(len(expected)):
        s = int(expected[i][0])
        e = int(expected[i][1])
        expected_indices.extend(range(s, e))

    # Get list of indices for the detected anomalies
    observed_indices = []
    for i in range(len(observed)):
        s = int(observed[i][0])
        e = int(observed[i][1])
        observed_indices.extend(range(s, e))

    # Iterate over the expected anomalies
    for i in range(start, end):
        if i in expected_indices and i in observed_indices:
            tp += 1
        elif i in expected_indices and i not in observed_indices:
            fn += 1
        elif i not in expected_indices and i in observed_indices:
            fp += 1
        else:
            tn += 1
...
def _overlap_segment(expected, observed, start=None, end=None):
    """
    Evaluate the confusion matrix between the ground truth and the detected anomalies.
    ...
    for expected_seq in expected:
        found = False
        # If the expected anomaly is detected, increment the true positive count
        for observed_seq in observed:
            if _overlap(expected_seq, observed_seq):
                ...
                if observed_seq in observed_copy:
                    # Remove the detected anomaly from the list
                    observed_copy.remove(observed_seq)
        # If the expected anomaly is not detected, increment the false negative count
        if not found:
            fn += 1
    # All remaining detected anomalies are false positives
...
def contextual_confusion_matrix(
    expected, observed, start, end, weighted=True
):
...
def _find_threshold(errors, z_range):
    ...
    for z in range(min_z, max_z):
        terrors = torch.tensor(errors)
        tmean = torch.tensor(mean)
        tstd = torch.tensor(std)
        best = fmin(z_cost, z, args=(terrors, tmean, tstd), full_output=True, disp=False)
        z, cost = best[0:2]
        if cost < best_cost:
            best_z = z[0]
            best_cost = cost
...
def _fixed_threshold(errors, k=4):
    ...
    The fixed threshold is based on the percentiles of the errors. The threshold is
    ...
    Returns:
        float:
            Calculated threshold value.
    """
    threshold = np.percentile(errors, 100 - 3)
    return threshold
...
def _merge_sequences(sequences):
    ...
    # Sort the sequences by start index
    sorted_sequences = sorted(sequences, key=lambda entry: entry[0])
    # Initialize the new sequences with the first element of the sorted list
    new_sequences = [sorted_sequences[0]] # [(start, end, score), ...]
    score = [sorted_sequences[0][2]] # [score, ...]
    weights = [sorted_sequences[0][1] - sorted_sequences[0][0]] # [weight, ...]

    # Iterate over the sorted list and merge overlapping or consecutive sequences
    for sequence in sorted_sequences[1:]:
        # Get the last sequence in the new list
        prev_sequence = new_sequences[-1]

        # If the start of the current sequence is less than or equal to the end of the last sequence
        if sequence[0] <= prev_sequence[1] + 1:
            # Add the score and weight of the current sequence to the lists
            score.append(sequence[2])
            weights.append(sequence[1] - sequence[0])
            if sum(weights) == 0:
                continue
            weighted_average = np.average(score, weights=weights)
            # Update the last sequence in the new list with the new end index and the weighted average
            new_sequences[-1] = (
                prev_sequence[0],
            ...
            # If the current sequence is not overlapping or consecutive, add it to the new list
            # and reset the score and weight lists
            score = [sequence[2]]
