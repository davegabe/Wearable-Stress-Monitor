...
def parse_args():
    ...
    parser.add_argument(
        "--epochs", default=70, type=int, help="number of epochs for the train"
    )
    parser.add_argument("--model", default="lstm", help="model: lstm / tadgan")
    parser.add_argument(
        "--hyperbolic", action="store_true", help="use hyperbolic space"
    )
    parser.add_argument(
        "--signal_shape", default=100, type=int, help="shape of the window"
    )
    parser.add_argument(
        "--id",
        default=1,
        type=int,
        help="id that tadgan has to consider [Dario's dataset only]",
    )
    ...
    parser.add_argument(
        ...
        default=1,
        type=int,
        help="which split to use for train/test of the CASAS dataset",
    )
    parser.add_argument(
        "--batch_size", default=64, type=int, help="number of samples per batch"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="print all the information"
    )
    parser.add_argument(
        "--save_result",
        action="store_true",
        help="save the output of confusion matrix at filename",
    )
    parser.add_argument("--filename", default="", help="name of the file with results")
    parser.add_argument(
        "--rec_error", default="dtw", help="name of the file with results"
    )
    parser.add_argument(
        "--combination", default="mult", help="name of the file with results"
    )
    parser.add_argument(
        "--interval", default=21600, type=int, help="used in the preprocessing phase"
    )
    parser.add_argument(
        "--unique_dataset", action="store_true", help="if train and test are different"
    )
    parser.add_argument(
        "--resume", action="store_true", help="true if you want to resume model"
    )
    parser.add_argument(
        "--resume_epoch", default=10, type=int, help="epoch you want to resume"
    )
    parser.add_argument("--load", action="store_true", help="enable load saved pickles")
    parser.add_argument(
        "--new_features", action="store_true", help="ony for the Dario's datasets"
    )
    parser.add_argument(
        "--config ", default="dtw", help="name of the file with results"
    )
